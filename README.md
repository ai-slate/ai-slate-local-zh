...
**架构概览：** 从统一入口到模型服务与调度，再到计价、调用记录和用量分析，展示一次 AI 调用如何在系统中完整流转。
...
智匣·融智可以把已经接入并达到可用状态的以下模型能力纳入统一网关。实际可调用范围以你配置的服务商、密钥、网络、余额和权限为准，接入后可通过一次最小真实调用完成确认。
...
统一的是：**网关入口、API Key 认证、模型选择、Provider Key 调度、Invocation 生命周期、用量、计价和文件生命周期**。不同能力沿用各自明确的请求格式，文本生成的三种协议也分别保持与对应生态兼容的字段结构。

在文本生成能力之上，智匣·融智还支持 **Claude Code、Codex 和 OpenCode Agent 接入**，可以作为三类 Coding Agent 的统一模型入口：Claude Code 使用 Anthropic Messages，Codex 使用 OpenAI Responses，OpenCode 使用 OpenAI-compatible Chat Completions。三套工具进入同一套 API Key 认证、模型选择、调度、用量与计价体系；具体配置与验证步骤请在产品内“在线接入文档”的“Coding Agent 接入”查看。为 Agent 选择具备工具调用能力的模型后，可先完成一次最小真实调用确认接入效果。
...
> 界面显示“已配置”后，可以通过一次最小真实调用继续确认该账户下目标模型的实际可用性。
...
智匣·融智本地版把这项工作收进一个产品化入口。以下能力都能在同一个管理页面里完成，并由可核对的运行机制支撑：
...
> 内置服务商目录帮助你快速选择计划接入的模型。添加服务商凭证并完成配置后，可通过一次最小真实调用确认目标模型在当前账号、网络和权限下的可用状态。
...
- 本地版采用 **SQLite 单机架构**，每个运行目录由一个实例独占，产品配置已经内嵌在二进制中，运行时无需加载同目录的 TOML/YAML。需要多实例、共享存储或企业部署时，可联系我们结合实际场景规划。
